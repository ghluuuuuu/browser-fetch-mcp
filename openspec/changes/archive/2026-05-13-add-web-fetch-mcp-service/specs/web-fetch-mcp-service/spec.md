...
The system SHALL provide a Python 3.13 MCP server that can run in SSE mode and streamable HTTP mode. The selected transport SHALL be configurable at runtime.

#### Scenario: Run with streamable HTTP
- **WHEN** the service starts with transport set to `streamable-http`
- **THEN** it SHALL expose MCP tools over the streamable HTTP endpoint

#### Scenario: Run with SSE
- **WHEN** the service starts with transport set to `sse`
- **THEN** it SHALL expose the same MCP tools over the SSE endpoint

### Requirement: Service MUST load runtime configuration
The system SHALL load runtime configuration for the Obscura CDP endpoint, MCP host, MCP port, transport mode, navigation timeout, batch concurrency, default search engine, and default result limits from configuration files and environment variables. Environment variables SHALL override file defaults.
