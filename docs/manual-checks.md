...
These checks require a running Chrome, Chromium, or compatible CDP server.

1. Start Chrome/Chromium with remote debugging enabled and note its CDP endpoint, for example `ws://127.0.0.1:9222`.
...
7. Call `search` with `keyword=chrome cdp` and `engine=bing`.
